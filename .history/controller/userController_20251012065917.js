...
async function register(req, res) {
    ...
    try {
        ...
        if (user.length > 0) {
            return res.status(StatusCodes.BAD_REQUES).json({ msg: "user already existed"})
        }

        if (password.length <= 8) {
            return res.status(StatusCodes.BAD_REQUES).json({ msg: "password must be at least 8 characters" })
        }
